...
This document describes all configuration options and settings available in the Ufanet intercom integration custom integration.
...
### `ufanet_intercom.example_service`
...
```yaml
service: ufanet_intercom.example_service
target:
  ...
  - alias: "Call service at sunset"
    trigger:
      - trigger: sun
        event: sunset
    action:
      - action: ufanet_intercom.example_service
        target:
          entity_id: switch.device_name_switch
        data:
          parameter: "mode"
...
```yaml
blueprint:
  name: Ufanet intercom integration Alert
...
- [GitHub Issues](https://github.com/nightmare04/ha_ufanet_intercom/issues) - Report problems
