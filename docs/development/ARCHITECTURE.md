...
This document describes the technical architecture of the Ufanet intercom integration custom component for Home Assistant.
...
│   ├── __init__.py          # Exports UfanetIntercomDataUpdateCoordinator
...
│   ├── __init__.py          # Exports UfanetIntercomEntity
...
- `base.py` - Main coordinator class (`UfanetIntercomDataUpdateCoordinator`)
...
**Key class:** `UfanetIntercomDataUpdateCoordinator` (exported from `coordinator/__init__.py`)
...
**Key class:** `UfanetIntercomApiClient`
...
- `UfanetIntercomConfigFlowHandler` (main flow)
- `UfanetIntercomOptionsFlow` (options)
...
**Key class:** `UfanetIntercomEntity` (in `entity/base.py`)
...
2. `UfanetIntercomEntity` for common functionality
...
1. Create directory: `custom_components/ufanet_intercom/<platform>/`
2. Implement `__init__.py` with `async_setup_entry()`
3. Create entity classes inheriting from platform base + `UfanetIntercomEntity`
