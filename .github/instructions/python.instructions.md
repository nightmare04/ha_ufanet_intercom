...
- Files: `snake_case.py`
- Classes: `PascalCase` prefixed with `UfanetIntercom`
...
- Inherit from both platform entity and `UfanetIntercomEntity` (order matters)
...
from custom_components.ufanet_intercom.sensor import async_setup_entry
...
**Config entry data:** `entry_data: UfanetIntercomData = hass.data[DOMAIN][entry.entry_id]`
