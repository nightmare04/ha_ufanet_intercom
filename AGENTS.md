...
- **Repository:** nightmare04/ha_ufanet_intercom

**Key directories:**

- `custom_components/ufanet_intercom/` - Main integration code
...
- Integration logs: `custom_components.ufanet_intercom: debug` in `config/configuration.yaml`
...
- Use the domain `ufanet_intercom` for all DOMAIN references
- Prefix all integration-specific classes with `UfanetIntercom`
- Use "Ufanet intercom integration" as the display title
...
- Inherit from platform base + `UfanetIntercomEntity`
...
- `tests/` mirrors `custom_components/ufanet_intercom/` structure
