"""
API Client for ufanet_intercom.
...
class UfanetIntercomApiClientError(Exception):
    """Base exception to indicate a general API error."""


class UfanetIntercomApiClientCommunicationError(
    UfanetIntercomApiClientError,
):
    """Exception to indicate a communication error with the API."""


class UfanetIntercomApiClientAuthenticationError(
    UfanetIntercomApiClientError,
):
...
def _verify_response_or_raise(response: aiohttp.ClientResponse) -> None:
    ...
    Raises:
        UfanetIntercomApiClientAuthenticationError: For 401/403 errors.
        aiohttp.ClientResponseError: For other HTTP errors.

    """
    if response.status in (401, 403):
        msg = "Invalid credentials"
        raise UfanetIntercomApiClientAuthenticationError(
            msg,
        )
    response.raise_for_status()


class UfanetIntercomApiClient:
    """
    ...
    async def async_get_data(self) -> Any:
        ...
        Raises:
            UfanetIntercomApiClientAuthenticationError: If authentication fails.
            UfanetIntercomApiClientCommunicationError: If communication fails.
            UfanetIntercomApiClientError: For other API errors.
    ...
    async def _api_wrapper(
        ...
        try:
            ...
            raise UfanetIntercomApiClientCommunicationError(
                msg,
            ) from exception
            ...
            raise UfanetIntercomApiClientError(
                msg,
            ) from exception
